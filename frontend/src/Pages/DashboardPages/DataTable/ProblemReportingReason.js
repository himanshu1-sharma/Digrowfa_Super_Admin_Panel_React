...
export default function StickyHeadTable() {
    ...
    const fetchData = () => {
        Axios.get(`${BASEURL}api/report/problem-issues`)
            .then(data => {
    ...
    const myFunction = async () => {
        try {
            await Axios.delete(`${BASEURL}delete-report-problem-issue/${selected}`)
...
function MyVerticallyCenteredModal(props) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            await Axios.post(`${BASEURL}save-report-problem-issue`, { categoryInput: input })
            props.onHide()
            setInput()
        ...
        >
            <Modal.Body>
                <div className='addCategoryModal'>
                    ...
                    <h4>Add User Reporting Reason</h4>
                    <form>
